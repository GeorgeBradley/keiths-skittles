...
from .forms import ScoreForm, PlayerSelectForm

def live_game(request, game_id=None):
    if not game_id:
        game = Game.objects.create(date="2025-03-23 19:00", opponent="St. Mary’s", location="The Red Lion", game_type="regular", cycles_per_round=4)
    ...
    if not round_players and request.method != "POST":
        return render(request, "scores/select_players.html", {
            "game": game, "players": Player.objects.all(), "current_round": current_round,
    ...
    return render(request, "scores/live_game.html", {
        "game": game, "keiths_scores": keiths_scores, "opp_scores": opp_scores,
        "current_round": current_round, "current_cycle": current_cycle,
        "round_players": round_players, "form": form,
        "keiths_cycle_total": keiths_cycle_total, "opp_cycle_total": opp_cycle_total,
        "keiths_round_total": keiths_round_total, "opp_round_total": opp_round_total,
        "keiths_game_total": keiths_game_total, "opp_game_total": opp_game_total
    })